...
parser.add_argument('--model', type=str,
                    default='weights/model_best.pth', help='trained model')
...
def detect_face(net, img, shrink):
    ...
    det_conf = detections[0, 1, :, 0] #[batch_size, cls_index, box,[conf,xmin,ymin,xmax,ymax]]
...
def get_data():
    subset = 'val'
    if subset == 'val':
        wider_face = sio.loadmat(os.path.join(cfg.HOME,'wider_face_split','wider_face_val.mat'))
    else:
...
if __name__ == '__main__':
 ...
 #  How to convert the train_model to an deploy model for test   
    #1. create an trian_model from train.pth
    train_net = build_s3fd_repvgg('train', cfg.NUM_CLASSES)
    train_net.load_state_dict(torch.load(args.model)['state_dict'])
    
  
   
    #2 convert tarin_model to deploy model by func "repvgg_model_convert"
    net = repvgg_model_convert(train_net, save_path = os.path.join(args.deploy_save,'S3FD_REPVGG_A1_deploy.pth'))
    #3 build an s3fd net which base is deploy vgg model
    net = build_s3fd_repvgg('test',cfg.NUM_CLASSES,deploy=True)
 
    net.eval()
    #4 load deploy_s3fd pth to test_model
    ...
    for index, event in enumerate(event_list):
        ...
        path = os.path.join(save_path, str(event[0][0]).strip())
        if not os.path.exists(path):
            os.makedirs(path)

        for num, file in enumerate(filelist):
            im_name = str(file[0][0]).strip()
